...
void action(node *root){
  ...
  if(root->left != nullptr){
    if(root->left->left == nullptr and root->left->right == nullptr) root->left = nullptr;
  }
  if(root->right != nullptr){
    if(root->right->left == nullptr and root->right->right == nullptr) root->right = nullptr;
  }
  action(root->left); action(root->right);
}
node* action2(node* root){
  if(root == nullptr) return nullptr;
  if(root->left == nullptr and root->right == nullptr) return nullptr;
  root->left = action2(root->left);
  root->right = action2(root->right);
  return root;
}
void task(node* root){
  // action(root);
